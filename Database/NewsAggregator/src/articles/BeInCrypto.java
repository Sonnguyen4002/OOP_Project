...
public class BeInCrypto extends WebScraper {
...
//        }
    ...
    protected void extractArticleDetails(Document doc) {
        ...
        OffsetDateTime publishDateTime = OffsetDateTime.parse(publishDateStr, DateTimeFormatter.ISO_OFFSET_DATE_TIME);
        publishDate = publishDateTime.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME);
        
        tags = extractCategoryFromLink(doc);
        author = doc.select("a[href*=author]").select("span.text-blue-700").text();
        category = extractCategoryFromLink(doc);
